...
class YumRegistrationController extends YumController {
	...
	public $textActivationBody =  
		'Hallo {username}.<br/><br/>Bitte aktiviere Deinen Account noch:: {activation_url}';
	...
	public function accessRules() {
		return array(
				array('allow',
					'actions' => array('index', 'registration', 'recovery', 'activation', 'resendactivation', 'regLehrer', 'regSchueler'),
					'users' => array('*'),
					),
				array('allow',
	...
	public function actionRegistration() {
		...
		if (isset($_POST['YumRegistrationForm'])) { 
			...
			if(!$form->hasErrors() && !$profile->hasErrors()) {
				...
				Yum::setFlash('Danke für Deine Registrierung ' .  CHtml::link('Hier gehts zur Aktivierung', $user->activationUrl));
    ...
    public function actionRegLehrer() {
        Yii::import('application.modules.profile.models.*');
		$form = new YumRegistrationForm;
		$profile = new YumProfile;
        
        $this->renderPartial('_regform_lehrer', array(
        	'form' => $form,
			'profile' => $profile,
        )
        );
    }
    public function actionRegSchueler() {
        $this->renderPartial('_regform_schueler');
    }
